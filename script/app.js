...
rollbackBtn.addEventListener('click', () => {
  ...
  bash += "# Find the FIRST painter init commit (--reverse = oldest first)\n";
  bash += 'INIT_COMMIT=$(git log --all --oneline --grep="Initialize repository for GitHub art" --format="%H" --reverse | head -1)\n';
  bash += "if [ -n \"$INIT_COMMIT\" ]; then\n";
  bash += '  echo "Found first painter commit: $INIT_COMMIT"\n';
  bash += '  PARENT=$(git rev-parse "$INIT_COMMIT^" 2>/dev/null || echo "")\n';
  bash += "  if [ -n \"$PARENT\" ]; then\n";
  bash += '    git reset --hard "$PARENT"\n';
  bash += '    echo "Restored to commit before first painter run"\n';
  bash += "  else\n";
  bash += '    git checkout --orphan clean-main\n';
  bash += '    echo "# Clean Repository" > README.md\n';
  bash += '    git add README.md\n';
  bash += '    git commit -m "Initial clean state"\n';
  bash += '    echo "Created clean initial state (repo was empty before painter)"\n';
  bash += "  fi\n";
  bash += "else\n";
