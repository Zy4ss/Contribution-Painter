...
pushBtn.addEventListener('click', () => {
  ...
  bash += "git pull origin main 2>/dev/null || git pull origin master 2>/dev/null\n";
  bash += "# Pakai email dari global git config biar kontribusi nyala\n";
  bash += 'GIT_EMAIL=$(git config --global user.email 2>/dev/null || echo "")\n';
  bash += 'GIT_NAME=$(git config --global user.name 2>/dev/null || echo "")\n';
  bash += 'if [ -z "$GIT_EMAIL" ]; then\n';
  bash += '  echo "WARNING: Git email not set. Contributions may not appear on GitHub."\n';
  bash += '  echo "Set it with: git config --global user.email \"your@email.com"\n';
  bash += '  GIT_EMAIL="temp@local"\n';
  bash += '  GIT_NAME="temp"\n';
  bash += "fi\n";
  bash += 'git config user.email "$GIT_EMAIL"\n';
  bash += 'git config user.name "$GIT_NAME"\n';
  bash += 'echo "Using email: $GIT_EMAIL"\n\n';
...
rollbackBtn.addEventListener('click', () => {
  ...
  bash += "git clone \"$REPO_URL\" . 2>&1\n";
  bash += "echo ''\n\n";
  bash += "# Hapus semua branch remote\n";
  bash += 'echo "Deleting all remote branches..."\n';
  bash += 'for br in $(git branch -r | grep -v HEAD | sed "s/origin\\///"); do\n';
  bash += '  git push origin --delete "$br" 2>&1\n';
  bash += "done\n";
  bash += "echo ''\n\n";
  bash += "# Buat clean initial state\n";
  bash += 'git checkout --orphan clean-main\n';
  bash += 'git rm -rf . 2>/dev/null\n';
  bash += 'rm -rf * .github .gitignore 2>/dev/null\n';
  bash += 'echo "# Clean Repository" > README.md\n';
  bash += 'git add README.md\n';
  bash += 'git commit -m "Initial clean state"\n';
  bash += 'git branch -M main\n\n';
  bash += 'echo "Force pushing clean state to main & master..."\n';
  bash += 'CURRENT_BRANCH=$(git rev-parse --abbrev-ref HEAD)\n';
  bash += 'git push -u origin "$CURRENT_BRANCH":main --force 2>&1\n';
  bash += 'git push -u origin "$CURRENT_BRANCH":master --force 2>&1\n';
  bash += "cd ..\n";
  bash += "rm -rf github_painter_undo_tmp\n";
  bash += "echo 'DONE!'\n";
